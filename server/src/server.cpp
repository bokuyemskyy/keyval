    ...
    while (m_running)
    {
        ...
        for (const auto &ev : poll.events())
        {
            if (ev.fd == listen_socket.fd()) // new connection
            {
                ...
                int client_socket_fd = accept(listen_socket.fd(), reinterpret_cast<sockaddr *>(&client_address), &client_len);
            ...
            else // existing connection
            {
                ...
                else // got data
                {
                    ...
                    {
                        std::string error_response = Protocol::serializeResponse(Response{
                            ResponseType::ERROR,
