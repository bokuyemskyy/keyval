...
public:
    Response execute(const Request &request, Session &session, Storage &storage) override final
    {
        if (request.args.size() != 1)
            return Response{ResponseType::ERROR, "ERR wrong number of arguments for 'KEYS'"};
        ...
        {
            responses.push_back(std::make_shared<Response>(
                Response{ResponseType::BULK_STRING, key}));
        }
