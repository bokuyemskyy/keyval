...
EventPoll::EventPoll(int max_events)
    ...
    if (m_event_fd == -1) throw std::runtime_error(strerror(errno));
...
void EventPoll::addFd(int fd, uint32_t events) {
    ...
    if (epoll_ctl(m_epoll_fd, EPOLL_CTL_ADD, fd, &ev) == -1) throw std::runtime_error(strerror(errno));
...
void EventPoll::modifyFd(int fd, uint32_t events) {
    ...
    if (epoll_ctl(m_epoll_fd, EPOLL_CTL_MOD, fd, &ev) == -1) throw std::runtime_error(strerror(errno));
    m_fd_events[fd] = events;
}
