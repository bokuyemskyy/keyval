   ...
   public:
    Response execute(const Request& request, Session& session, Storage& storage) override final {
        if (request.args.size() != 2)
            return Response{ResponseType::ERROR, "ERR wrong number of arguments for 'EXPIRE'"};
        const std::string& key = request.args[0];
        int                seconds;
        try {
            seconds = std::stoi(request.args[1]);
        } catch (const std::invalid_argument&) {
            return Response{ResponseType::ERROR, "ERR value is not an integer or out of range"};
        }
